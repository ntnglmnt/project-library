...
const library = {
    ...
    storeBook: function () {
        ...
        library.libraryArray.push(newBook)
        console.log(this.libraryArray)
    },

    displayBook: function () {
        library.libraryArray.map((item) => {
            item.showBook()
        })
    },
...
function createLibraryParts() {
    ...
    const add = document.createElement('button')
    add.textContent = 'Add'
    add.addEventListener('click', library.storeBook)
    document.body.appendChild(add)

    const displayBtn = document.createElement('button')
    displayBtn.classList.add('displayBtn')
    displayBtn.textContent = 'Display'
    displayBtn.addEventListener('click', library.displayBook)
    display.appendChild(displayBtn)
}

let libraryInitialisation = createLibraryParts
libraryInitialisation()
